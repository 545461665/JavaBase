...
![170、多线程的引入](F:\01、java基础\笔记的截图图片\170、多线程的引入.png)

## 2、进程概述和多线程的意义

```
什么是进程？
	进程：就说正在运行的程序，是系统进行资源分配和调用的对单位，每一个进程都有他自己的内存空间和系统资源
	
多线程的意义：
	单进程的计算机只能做一件事情，而我们现在的计算机都可以做多件事情：
		一边玩游戏（游戏进程）一边听音乐（音乐进程）
	也就是说现在的计算机都是支持多进程的，就是在一个时间段内执行多个任务并且可以提高CPU的使用率
...
问题：
	一边玩游戏，一边听音乐是同时进行的吗？
		...
		而我们在玩游戏或者听音乐的时候，是CPU在做着进程间的高效切换让我们觉得同时进行的。
```

### 2.1、什么是线程

> 在同一个进程内有可以执行多个任务，而每一个任务就可以看成是一个线程
>
...
多线程的存在，不是提高程序的执行速度，其实是为了提高应用程序的使用率。
	程序的执行其实都是在强CPU的资源，CPU的执行权，多个进程是在抢这个资源，而其中的某一个进程如果执行路径比较	   多，就会有更高的几率抢到CPU的执行权。
...
```
并行：在逻辑上同时发生，指在某一个时间内（时间段）同时运行多个程序
并发：在物理上同事发生，只在某一个时间点同时运行多个程序
```
...
思考：
	Jvm虚拟机的启动时单线程还是多线程？
		是多线程的。因为垃圾回收线程也要先启动，否自很容易会出现内存溢出，现在的垃圾回收线程+前面的主线程，		  最低启动了两个线程。
```

## 4、如何实现多线程

```
由于线程是依赖进程而存在的，所以我们应该先创建一个进程出来，而进程时候系统创建的所以我们应该去调用系统功能创建一个进程。java是不能直接调用系统功能的，所以我们没有办法直接实现多线程。
但是java可以调用C/C++写好的程序来实现多进程程序。由C/C++去调用系统功能创建进程，然后由java去调用，然后提供一些类供我们使用，就可以实现多线程程序了
```



### 4.1、多线程实现方式1：继承Thread类：
...
继承Thread类：
	...
	B:MyThread类里面重写run()
		为什么是run()方法？
			不是类中所有代码都需要被线程执行，而这个时候为了区分哪些代码能够被线程执行，java提供了THread				类中的run()用来包含哪些被线程执行的代码
		调用run()方法为什么是单线程？
			因为run方法直接调用，其实就是普通的方法调用，所以是单线程的效果
			想要看到多线程的效果，就必须使用start()方法
	C:创建对象
	D:启动线程
```

![171、多线程实现方式_继承Thread类](F:\01、java基础\笔记的截图图片\171、多线程实现方式_继承Thread类.png)
...
public final Strign setName(String name):设置线程名称
```

![172、获取和设置线程对象名称](F:\01、java基础\笔记的截图图片\172、获取和设置线程对象名称.png)

```
获取main方法所在对象的名称，该怎么办
	public static Thread currentThread():返回当前正在执行的线程对象
在MyThreadDemo类中的main方法直接输出即可，代码如下
...
```
所有线程轮流使用cup的使用权，平均分配每个线程占用cpu的时间片
```

##### B、抢占式调度模型

```
优先让优先级高的线程使用CPU，如果线程的优先级相同，那么会随机算则一个，优先级的线程获取的CPU时间片相对多一些
```
...
public final void setPriority(int newPriority)：更改线程对象优先级
```

##### ![173、线程调度及获取和设置线程优先级](F:\01、java基础\笔记的截图图片\173、线程调度及获取和设置线程优先级.png)

```
注意：线程优先级仅仅表示线程获取CUP时间片的几率高，但是要在次数比较多，多次运行的时候才能看到比较好的效果，因为他存在随机性
```

### 4.4、多线程实现方式2：实现runnable接口

![176、多线程方式2_实现Runnable接口](F:\01、java基础\笔记的截图图片\176、多线程方式2_实现Runnable接口.png)

### 4.5、多线程两种方式的图解比较和区别

![177、多线程两种方式的图解比较和区别](F:\01、java基础\笔记的截图图片\177、多线程两种方式的图解比较和区别.png)
...
![174、线程控制_休眠](F:\01、java基础\笔记的截图图片\174、线程控制_休眠.png)
...
![174、线程控制_加入](F:\01、java基础\笔记的截图图片\174、线程控制_加入.png)
...
![174、线程控制_礼让](F:\01、java基础\笔记的截图图片\174、线程控制_礼让.png)
...
![174、线程控制_守护](F:\01、java基础\笔记的截图图片\174、线程控制_守护.png)

### 5.5、中断线程*

```
public final void stop():让线程停止，过时了，但是还是可以使用
public void interrupt():中断线程
```

![174、线程控制_终止](F:\01、java基础\笔记的截图图片\174、线程控制_终止.png)

## 6、线程的生命周期

![175、线程的生命周期](F:\01、java基础\笔记的截图图片\175、线程的生命周期.png)
...
```
共有100张票，而又3个销售窗口售票，设计一个程序模拟该电影院售票
```
...
![178、练习_继承Thread](F:\01、java基础\笔记的截图图片\178、练习_继承Thread.png)
...
![178、练习_实现runnable接口](F:\01、java基础\笔记的截图图片\178、练习_实现runnable接口.png)

```
实际生活中，卖票是有延迟时间的，所以改进代码
```

![178、卖票出现同票和负数票](F:\01、java基础\笔记的截图图片\178、卖票出现同票和负数票.png)
